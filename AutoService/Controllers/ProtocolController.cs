...
namespace AutoServiceWebSite.Controllers
{
    ...
    public class ProtocolController : Controller
    {
        ...
        public ActionResult Add(int id = 0)
        {
            ...
            var queue = id > 0 ? _repoWrapper.Queue.GetIncomplete(placeId.Value) : _repoWrapper.Queue.GetNextInLineByPlace(placeId.GetValueOrDefault());
            ...
            var options = _repoWrapper.OptionQueue.Query().Where(oq => oq.QueueId == queue.QueueId).ToList();
            for (int i = 0; i < options.Count; i++)
            {
                optionName += _repoWrapper.Option.GetById(options[i].OptionId).Name;
